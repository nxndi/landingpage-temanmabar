...
export type TButtonVariants = "solid" | "outline" | "default" | "custombg";
...
const Button = forwardRef<HTMLButtonElement, IButtonProps>((props, ref) => {
  ...
  const btnVariants: { [key in TButtonVariants]: string } = {
    solid: classNames(
      ...
      [`hover:border-${borderColor}-${shadeColorIntensity as TColorIntensity}`],
      // Active
      [`active:bg-${color}-${shadeColorIntensity as TColorIntensity}`],
      [
        `active:border-${borderColor}-${
          shadeColorIntensity as TColorIntensity
        }`,
      ],
      {
        [`bg-${color}-${shadeColorIntensity as TColorIntensity}`]: isActive,
        [`border-${borderColor}-${shadeColorIntensity as TColorIntensity}`]:
          isActive,
      }
    ),
    outline: classNames(
      ...
      [`${borderWidth}`],
      {
        [`border-${borderColor}-${colorIntensity}/50`]: !isActive,
      },
      "text-black dark:text-white",
      // Hover
      [`hover:border-${borderColor}-${colorIntensity}`],
      // Active
      [`active:border-${borderColor}-${colorIntensity}`],
      {
        [`border-${borderColor}-${colorIntensity}`]: isActive,
      }
    ),
    custombg: classNames(
      // Default
      ...
      // Hover
      [
        `hover:bg-${color}-${colorIntensity} hover:dark:bg-${color}-${colorIntensity}`,
      ],
      // Active
      [
        `active:bg-${color}-${colorIntensity} active:dark:bg-${color}-${colorIntensity}`,
      ],
      {
        [`bg-${color}-${colorIntensity}`]: isActive,
      }
  ...
  const btnRightIconClasses = HAS_CHILDREN
    ? btnSizes[size].rightIcon
    : undefined;
  ...
  return (
    ...
    >
      ...
      {children}
      {!!rightIcon && (
        <Icon icon={rightIcon} className={classNames(btnRightIconClasses)} />
      )}
    </button>
  );
});
