...
const BackDrop = () => {
  ...
  return (
    <motion.div
      ...
      {...animationProps}
      className="fixed left-0 top-0 z-[1050] h-screen w-screen bg-black/50 backdrop-blur-sm"
    />
  );
};
...
const Content: FC<IContentProps> = (props) => {
  ...
  const classes = classNames(
    "pointer-events-auto relative flex w-full flex-col bg-accent-200 dark:bg-accent-800 dark:text-white text-zinc-950",
    "shadow-2xl",
    [`${rounded}`],
    {
...
const Dialog = forwardRef<HTMLDivElement, IDialogProps>((props, ref) => {
  ...
  const classes = classNames(
    "pointer-events-none relative mx-auto my-6 max-w-[var(--theme-modal-width)] w-full",
    {
      ...
      "flex min-h-[calc(100%-theme(margin.6)*2)] items-center":
  ...
  return (
    <div
      data-component-name="Modal/Dialog"
      ref={ref}
      className={classes}
      {...rest}
    >
...
const Modal: FC<IModalProps> = (props) => {
  ...
  const closeModal = (event: { target: any }) => {
    ...
    const isInsideAnyModal = // @ts-expect-error
      ref.current?.contains(event.target);

    if (!isInsideAnyModal && !isStaticBackdrop && !isNestedOpen) {
      setIsOpen(false);
    }
  };
          ...
          <>
            ...
            <motion.div
              ...
              className={classNames(
                "fixed left-0 top-0 z-[1055] block h-full w-full overflow-y-auto overflow-x-hidden px-5 md:px-0",
                {
                  [`${appConfig.transition}`]: isStaticBackdrop,
                }
              )}
              role="dialog"
              tabIndex={-1}
